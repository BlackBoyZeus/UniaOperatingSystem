#!/bin/bash
# Final UNIA OS QEMU Boot Script with nightly Rust toolchain
...
if [ ! -f "$BINARY_PATH" ]; then
    echo -e "${YELLOW}Binary not found. Attempting to build with nightly Rust...${NC}"
    
    # Navigate to boot directory
    cd "$BOOT_DIR"
    
    # Build the bootable image with the nightly cross-compilation script
    echo -e "${YELLOW}Building with nightly Rust toolchain...${NC}"
...
qemu-system-x86_64 $QEMU_ARGS \
    -m 1G \
    -smp 4 \
    -drive format=raw,file="$BINARY_PATH" \
    -vga std \
